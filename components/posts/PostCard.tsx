...
export function PostCard({ post }: PostCardProps) {
  // Handle both frontend and backend field naming
  const replyCount = post.repliesCount ?? post.replyCount ?? 0;
  const retweetCount = post.repostsCount ?? post.retweetCount ?? 0;
  const likeCount = post.likesCount ?? post.likeCount ?? 0;
  
  // Handle posts that might not have a user object (from backend)
